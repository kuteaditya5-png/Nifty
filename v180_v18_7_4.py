...
def _bt_indicators(x):
    ...
    # Index candles may have no usable traded volume. Do not create all-NaN VWAP.
    # Use session cumulative typical-price proxy only for signal diagnostics until NIFTY FUT is connected.
    vol_ok=pd.to_numeric(x.volume,errors="coerce").fillna(0).gt(0).any()
    if vol_ok:
        x["vwap"]=(typical*x.volume).groupby(day).cumsum()/x.volume.groupby(day).cumsum().replace(0,np.nan)
        x["volavg"]=x.volume.rolling(20).mean()
        x["volume_confirm"]=x.volume>x["volavg"]
        x.attrs["vwap_source"]="NIFTY candle volume"
    else:
        x["vwap"]=typical.groupby(day).expanding().mean().reset_index(level=0,drop=True)
        x["volavg"]=np.nan
        x["volume_confirm"]=True
        x.attrs["vwap_source"]="PRICE-ONLY PROXY (NIFTY Futures VWAP/volume pending)"
    return x
...
<div class="footer">V18.7.4 • Paper trading only • EMA crossover uses completed 5-minute candles • Backtest reports NIFTY underlying points, not option-premium P&amp;L.</div></div>
<script>
