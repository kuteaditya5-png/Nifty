...
def _filtered_v2_backtest(x):
    ...
    for i in range(55,len(x)-1):
        ...
        diag["rsi_band"]+=1
        if not (pd.notna(r.volavg) and r.volume>r.volavg): continue
        diag["volume"]+=1
...
<div class="footer">V18.7.3 • Paper trading only • EMA crossover uses completed 5-minute candles • Backtest reports NIFTY underlying points, not option-premium P&amp;L.</div></div>
<script>
