...
def _real_option_backtest(filtered):
    ...
    try:
        ...
        return {"status":"unavailable","message":f"Option contract lookup failed: {e}","history":[],
            "diagnostics":{"priced_trades":0,"skipped_trades":0,"failures":[{"reason":str(e)}]}}
    ...
    return {"status":"success" if out else "unavailable","message":None if out else "No trades could be priced with option candles",
        ...
        "diagnostics":{"priced_trades":len(out),"skipped_trades":len(skipped),"failures":skipped[:10]}}
...
def _ema_cross_backtest(days=45, from_date=None, to_date=None):
    ...
    result.update({"status":"success","version":VERSION,"strategy":"FILTERED V2","actual_from":actual_from,"actual_to":actual_to,
         ...
         "pnl_mode":"ACTUAL OPTION OHLC" if primary is option_bt else "UNDERLYING PROXY",
        "option_diagnostics":option_bt.get("diagnostics",{"failures":option_bt.get("skipped",[])}),
        "option_message":option_bt.get("message"),"vwap_source":x.attrs.get("vwap_source","unknown"),
...
<body>
<div id="v1881nav" style="position:sticky;top:0;z-index:9999;background:#07131f;border-bottom:1px solid #223247;padding:8px 22px;display:flex;gap:10px;align-items:center">
  <button onclick="location.href='/dashboard'" style="background:#17283a;color:#e5eef8;border:1px solid #36506a;border-radius:8px;padding:8px 14px;cursor:pointer">⌂ Dashboard</button>
  <button disabled style="background:#0e7490;color:white;border:1px solid #22a6c3;border-radius:8px;padding:8px 14px">⚡ Intraday</button>
</div><div class="head"><div class="brand">NIFTY <b>AI</b></div><div class="live">● Paper engine</div></div>
...
<div style="margin-top:12px"><b>Option OHLC Validation</b>
<div id="optdiag" class="paperGrid" style="margin-top:8px"></div>
<div id="opterror" class="muted" style="margin-top:6px"></div></div>
...
<div class="footer">V18.8.1 • Paper trading only • Dashboard restored • option OHLC diagnostics • no order placement • EMA crossover uses completed 5-minute candles • Backtest reports NIFTY underlying points, not option-premium P&amp;L.</div></div>
<script>
