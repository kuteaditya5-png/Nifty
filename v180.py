...
def _bt_indicators(x):
    ...
    # 15-minute trend state built only from completed 15m bars and merged backward.
    q=x.set_index("ts").resample("15min").agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna().reset_index()
    q["ema20_15"]=q.close.ewm(span=20,adjust=False).mean(); q["ema50_15"]=q.close.ewm(span=50,adjust=False).mean()
    x=pd.merge_asof(x.sort_values("ts"),q[["ts","ema20_15","ema50_15"]].sort_values("ts"),on="ts",direction="backward")
    return x

def _simulate_strategy(x, filtered=False):
    start_capital=50000.0; capital=start_capital; peak=capital; maxdd=0.0; trades=[]; daily={}
    pos=None
    for i in range(55,len(x)-1):
        r=x.iloc[i]; nxt=x.iloc[i+1]; t=r.ts
        tm=t.time(); d=t.date()
        if d not in daily: daily[d]={"start":capital,"trades":0,"sl":0}
        # Manage open underlying-risk proxy. True option-premium OHLC is still required for premium hit reconstruction.
        if pos:
            side=pos["side"]; stop=pos["stop"]; t1=pos["t1"]; t2=pos["t2"]
            sl_hit=(r.low<=stop) if side=="CE" else (r.high>=stop)
            t1_hit=(r.high>=t1) if side=="CE" else (r.low<=t1)
            t2_hit=(r.high>=t2) if side=="CE" else (r.low<=t2)
            ambiguous=sl_hit and (t1_hit or t2_hit)
            reason=None; exitp=None
            if ambiguous: reason="AMBIGUOUS→SL"; exitp=stop; sl_hit=True; t1_hit=False; t2_hit=False
            elif sl_hit: reason="STOP LOSS"; exitp=stop
            elif t2_hit: reason="TARGET 2"; exitp=t2; t1_hit=True
            ...
            if reason:
                ...
                risk_cash=pos["risk_cash"]; risk_pct=abs((pos["entry"]-stop)/pos["entry"])
                allocation=min(capital, risk_cash/risk_pct) if risk_pct>0 else 0
                pnl=allocation*move; capital+=pnl; peak=max(peak,capital); maxdd=max(maxdd,peak-capital)
                if "STOP" in reason: daily[d]["sl"]+=1
                trades.append({"time":str(pos["time"]),"side":side,"entry":round(pos["entry"],2),"stop_loss":round(stop,2),
                    "target1":round(t1,2),"target2":round(t2,2),"target1_hit":bool(t1_hit),"target2_hit":bool(t2_hit),
                    "stop_loss_hit":bool(sl_hit),"exit":round(exitp,2),"pnl":round(pnl,2),"balance":round(capital,2),"reason":reason})
                pos=None
            continue
        if tm<time(9,25) or tm>time(11,15): continue
        if daily[d]["trades"]>=2 or daily[d]["sl"]>=2 or capital<=daily[d]["start"]*0.98: continue
        if filtered:
            ce=(r.ema20>r.ema50 and r.close>r.vwap and 55<=r.rsi<=75 and r.volume>r.volavg and r.close>r.ph3
                and r.adx>20 and r.ema20_15>r.ema50_15 and abs(r.close-r.vwap)/r.vwap<=0.004)
            pe=(r.ema20<r.ema50 and r.close<r.vwap and 25<=r.rsi<=45 and r.volume>r.volavg and r.close<r.pl3
                and r.adx>20 and r.ema20_15<r.ema50_15 and abs(r.close-r.vwap)/r.vwap<=0.004)
        else:
            ce=(r.ema20>r.ema50 and x.iloc[i-1].ema20<=x.iloc[i-1].ema50)
            pe=(r.ema20<r.ema50 and x.iloc[i-1].ema20>=x.iloc[i-1].ema50)
        if not (ce or pe): continue
        side="CE" if ce else "PE"; entry=float(nxt.open)
    ...
    return {"starting_capital":start_capital,"ending_capital":round(capital,2),"net_pnl":round(capital-start_capital,2),
        "return_percent":round((capital/start_capital-1)*100,2),"trades":len(trades),"wins":wins,"losses":len(trades)-wins,
        ...
        "stop_loss_hits":sum(t["stop_loss_hit"] for t in trades),"history":trades[-100:]}
...
def _ema_cross_backtest(days=45, from_date=None, to_date=None):
    ...
    baseline=_simulate_strategy(x,False); filtered=_simulate_strategy(x,True)
    ...
    result.update({"status":"success","version":VERSION,"strategy":"FILTERED V2","actual_from":actual_from,"actual_to":actual_to,
        "trading_days":trading_days,"baseline":baseline,"filtered":filtered,
        "note":"Filtered V2: 15m trend + EMA state + VWAP + ADX>20 + RSI band + volume-confirmed 3-bar breakout, max 0.4% from VWAP, 1% risk/trade, max 2 trades/day, -2% daily stop. Results currently use NIFTY underlying OHLC as the signal/risk proxy; historical option-premium OHLC is still required for true option ₹ P&L."})
    return result
...
<div class="footer">V18.7.2 • Paper trading only • EMA crossover uses completed 5-minute candles • Backtest reports NIFTY underlying points, not option-premium P&amp;L.</div></div>
<script>
...
function clearBT(){['btend','btn','btret','btt','btw','btt1','btt2','btsl','btdd'].forEach(id=>$(id).textContent='--');$('bth').innerHTML='';$('btcompare').innerHTML='';$('btperiod').textContent='Period: --'}
