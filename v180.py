...
def _calc(df):
    ...
    c=x.iloc[-1]
    required={"close":c.close,"ema20":c.ema20,"ema50":c.ema50,"rsi14":c.rsi14,"vwap":c.vwap,"volume":c.volume,"volavg":c.volavg,"prior_high":c.prior_high,"prior_low":c.prior_low}
    ...
    candle_time=c.ts.time(); is_today=c.ts.date()==datetime.now(IST).date(); window=is_today and time(9,25)<=candle_time<=time(11,15); vol=bool(c.volume>c.volavg)
    ...
    sig="NO TRADE"; setup_state="NO TRADE"; checks=bull if sum(bull.values())>=sum(bear.values()) else bear
