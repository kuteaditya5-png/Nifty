...
def setup_v15291(app):
    ...
    def prepare():
        from main import _v146_feature_frame_from_raw
        raw=_merged_raw()
        f=_v146_feature_frame_from_raw(raw)
        if f is None or len(f)<800:return None,"Insufficient NIFTY 15-minute history.",len(raw)
        x=f.copy().sort_index()
        ...
        if not all([ccol,hcol,lcol]):return None,"OHLC columns not found.",len(raw)
        ...
        x=x.replace([np.inf,-np.inf],np.nan).dropna(subset=["trend_strength_v1523","atr14_pct_v1523","rv20_v1523","direction_v1523","fwd_bps_v1523"])
        return x,None,len(raw)

    @app.get("/api/regime/v15291-status")
    def status():
        try:
            x=_merged_raw()
            return {"status":"success","version":VERSION,"merged_unique_rows":len(x),"target_rows":TARGET,
                    "ready":len(x)>=TARGET,"validator":"/api/regime/frozen-validation-v15291",
                    "runtime_fix_only":True,"frozen_rule_changed":False,"live_routing_changed":False}
        except Exception as e:
            return {"status":"error","version":VERSION,"stage":"STATUS","error_type":type(e).__name__,"message":str(e),"live_routing_changed":False}

    @app.get("/api/regime/frozen-validation-v15291")
    def validate(blocks:int=4,cost_bps:float=3.0):
      try:
        df,err,merged_rows=prepare()
        if err:return {"status":"error","version":VERSION,"stage":"PREPARE","message":err,"merged_unique_rows":merged_rows,"live_routing_changed":False}
        ...
        for i in range(blocks):
            ...
            folds.append(r)
        if not oos:return {"status":"error","version":VERSION,"stage":"VALIDATION","message":"No frozen TREND_LOW_VOL signals generated.","folds":folds,"live_routing_changed":False}
        ...
        return {"status":"success","version":VERSION,"integrated_history":{"merged_unique_rows":merged_rows},"hypothesis":{"regime":"TREND_LOW_VOL","direction":"EMA20 vs EMA50","horizon_bars":6,"timeframe":"15m","cost_bps":cost_bps,"regime_cutoffs":"60th percentiles learned from training history only","signal_independence":"every 6th eligible observation"},"verdict":"FROZEN TREND_LOW_VOL HYPOTHESIS PASSES" if passed else "FROZEN TREND_LOW_VOL HYPOTHESIS REJECTED","summary":summary,"folds":folds,"gate_checks":gates,"runtime_fix_only":True,"frozen_rule_changed":False,"next_action":"Eligible for final validation only; keep live routing unchanged." if passed else "Reject this frozen regime hypothesis; do not tune it to force a pass. Keep live routing unchanged.","live_routing_changed":False}
      except Exception as e:
        return {"status":"error","version":VERSION,"stage":"RUNTIME","error_type":type(e).__name__,"message":str(e),"trace_tail":traceback.format_exc().splitlines()[-8:],"runtime_fix_only":True,"frozen_rule_changed":False,"live_routing_changed":False}
